RSpec.describe ForecastOverview do
  let(:overview) { ForecastOverview.new(activity) }

  let :histories do
    Hash.new do |hash, (year, quarter)|
      hash[[year, quarter]] = ForecastHistory.new(activity, financial_quarter: quarter, financial_year: year)
    end
  end
  ...
  context "for a level C activity, with report-pinned history" do
    ...
    context "when there are forecasts for multiple activities" do
      let(:project) { create(:project_activity, parent: activity.parent, organisation: partner_organisation) }
      let(:project_history) { ForecastHistory.new(project, financial_quarter: 1, financial_year: 2019) }
      let(:project_overview) { ForecastOverview.new(project) }
  ...
  context "for a set of activities across different levels" do
    ...
    let(:overview) { ForecastOverview.new([programme.id] + programme.descendants.pluck(:id)) }

    let :forecasts do
      Hash.new do |hash, key|
        activity, year, quarter = key
        hash[key] = ForecastHistory.new(activity, financial_year: year, financial_quarter: quarter)
      end
    end

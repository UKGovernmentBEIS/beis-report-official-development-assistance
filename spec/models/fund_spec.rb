...
RSpec.describe Fund, type: :model do
  it "sets the default value of title" do
    fund = Fund.new
    expect(fund.title).to eq "Untitled fund"
  end
  ...
  describe "validations" do
    ...
    context "when planned_start_date is not blank" do
      ...
      it "does not allow a planned_start_date more than 10 years ago" do
        fund = build(:fund, planned_start_date: 11.years.ago)
        expect(fund.valid?).to be_falsey
        expect(fund.errors[:planned_start_date]).to include "Date must be between 10 years ago and 25 years in the future"
      end

      it "does not allow a planned_start_date more than 25 years in the future" do
        fund = build(:fund, planned_start_date: 26.years.from_now)
        expect(fund.valid?).to be_falsey
      end

      it "allows a planned_start_date between 10 years ago and 25 years in the future" do
        fund = build(:fund, planned_start_date: Date.today)
        expect(fund.valid?).to be_truthy
      end
    end

    context "when recipient_region is blank" do
      subject { build(:fund, recipient_region: nil, wizard_status: :country) }
      it { should validate_presence_of(:recipient_region) }
    end

    context "when flow is blank" do
      subject { build(:fund, flow: nil, wizard_status: :flow) }
      it { should validate_presence_of(:flow) }
    end

    context "when finance is blank" do
      subject { build(:fund, finance: nil, wizard_status: :finance) }
      it { should validate_presence_of(:finance) }
    end

    context "when tied_status is blank" do
      subject { build(:fund, tied_status: nil, wizard_status: :tied_status) }
      it { should validate_presence_of(:tied_status) }
    end

    context "when the wizard_status is complete" do
      subject { build(:fund, wizard_status: "complete") }
      it { should validate_presence_of(:title) }
      it { should validate_presence_of(:description) }
      it { should validate_presence_of(:sector) }
      it { should validate_presence_of(:status) }

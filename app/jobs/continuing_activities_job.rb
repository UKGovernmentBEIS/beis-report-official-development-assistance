class ContinuingActivitiesJob < ApplicationJob
  require "csv"

  def perform(requester_id:)
    requester = User.find(requester_id)

    export = Export::ContinuingActivities.new
    upload = upload_csv_to_s3(file: save_tempfile(export), filename: export.filename)

    download = Download.where(purpose: "continuing_activities").last
    if !download
      download = Download.new
      download.purpose = "continuing_activities"
    end

    download.filename = upload.timestamped_filename
    download.save!

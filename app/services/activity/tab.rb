class Activity
  class Tab
    ...
    def financials
      @transactions = policy_scope(Actual).where(parent_activity: @activity).order("date DESC")
      ...
      @transaction_presenters = @transactions.includes(:parent_activity).map { |actual| TransactionPresenter.new(actual) }

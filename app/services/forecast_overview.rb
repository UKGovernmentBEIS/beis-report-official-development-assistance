class ForecastOverview
  LATEST_ENTRY_PER_ACTIVITY_AND_QUARTER = <<~SQL
    DISTINCT ON (
      forecasts.parent_activity_id,
      forecasts.financial_year,
      forecasts.financial_quarter
    )
    forecasts.*
  SQL
  ...
  def latest_with_report_versioning
    historical_entries
      ...
      .order(forecast_type: :desc)
  end

  def latest_unversioned
    historical_entries.order(forecast_type: :desc)
  end
  ...
  class Snapshot
    def self.non_zero_forecasts_from(relation)
      Forecast.unscoped.from(relation, :forecasts).where.not(value: 0)
    end
